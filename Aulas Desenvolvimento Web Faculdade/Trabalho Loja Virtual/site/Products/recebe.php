...
<head>
	<meta charset="utf-8">
	<title>Cell Bytes Localiza Produto</title>
...
<body>
	<section id="produtos">
		<h2>Produtos</h2>
		<div align = "left">
				<a href="index.php" target="_self"><input type="submit" value="PESQUISAR OUTRO PRODUTO!" class="button"> </a>
		</div>
		<?php
			$nameProduct	  = $_POST["prodName"];
			require("../banco/conecta_banco.php");
			$link = mysqli_connect("localhost", "root", "", "cellBytes");
			$result = mysqli_query($link, "SELECT id, nameProduct, imageAdress1, imageAdress2, descricao, price from product where nameProduct like '%$nameProduct%'");
			$foi = false;
			...
			if(!$foi){
				echo "<h2> Produtos não cadastrados! </h2>";
			}
		?>
	</section>		
	<br><br><br>
